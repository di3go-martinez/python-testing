...
class TestcontainersID(Scalar):
    """The `TestcontainersID` scalar type represents an identifier for an
    object of type Testcontainers."""
...
class Directory(Type):
    ...
    def as_module(self, *, source_root_path: str | None = ".", engine_version: str | None = None,) -> "Module":
...
class Module(Type):
    ...
    def with_source(self, source: "ModuleSource", *, engine_version: str | None = None,) -> Self:
...
class ModuleSource(Type):
    ...
    def as_module(self, *, engine_version: str | None = None,) -> Module:
    ...
    def resolve_directory_from_caller(self, path: str, *, view_name: str | None = None, ignore: list[str] | None = None,) -> Directory:
...
class Client(Root):
    ...
    def load_testcontainers_from_id(self, id: TestcontainersID) -> "Testcontainers":
        """Load a Testcontainers from its ID."""
        _args = [
            Arg("id", id),
        ]
        _ctx = self._select("loadTestcontainersFromID", _args)
        return Testcontainers(_ctx)
    ...
    def module_source(self, ref_string: str, *, ref_pin: str | None = "", stable: bool | None = False, rel_host_path: str | None = "",) -> ModuleSource:
    ...
    def testcontainers(self) -> "Testcontainers":
        _args: list[Arg] = []
        _ctx = self._select("testcontainers", _args)
        return Testcontainers(_ctx)
...
@typecheck
class Testcontainers(Type):
    """ Testcontainers provides a simple interface for wrapping an
    existing test suite that uses Testcontainers.  """

    def docker(self) -> Service:
        _args: list[Arg] = []
        _ctx = self._select("docker", _args)
        return Service(_ctx)

    def docker_service(self) -> Service:
        """
        DockerService exposes the Docker service so that you can start it
        before
        running a bunch of test suites, keeping it around across the full run
        even
        if there is excessive idle time due to load.
        """
        _args: list[Arg] = []
        _ctx = self._select("dockerService", _args)
        return Service(_ctx)

    async def id(self) -> TestcontainersID:
        """A unique identifier for this Testcontainers.
        ...
        _args: list[Arg] = []
        _ctx = self._select("id", _args)
        return await _ctx.execute(TestcontainersID)

    def setup(self, ctr: Container) -> Container:
        """
        Setup attaches a Docker daemon to the container and points
        Testcontainers to
        it.
        """
        _args = [
        ...
        _ctx = self._select("setup", _args)
        return Container(_ctx)

    def with_docker(self, docker: Service) -> Self:
        """
        WithDocker allows you to override the Docker daemon used by
        Testcontainers.
        """
        _args = [
            Arg("docker", docker),
        ]
        _ctx = self._select("withDocker", _args)
        return Testcontainers(_ctx)

    def with_(self, cb: Callable[["Testcontainers"], "Testcontainers"]) -> "Testcontainers":
        """Call the provided callable with current Testcontainers.
